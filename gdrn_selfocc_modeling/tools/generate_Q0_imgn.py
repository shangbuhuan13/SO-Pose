...
def estimate_occ_mask_Q0(rootdir, modeldir, cls_name, scale=1000):  # cls_name也就是objectname
    # create basic path if not exist
    basic_path = os.path.join(rootdir, "Q0", cls_name)
    if not os.path.exists(basic_path):
        os.makedirs(basic_path)

    img_dir = os.path.join(rootdir, "imgn", cls_name)
    img_num = len(os.listdir(img_dir)) // 30  # 得到图片的数量
    model_info_path = os.path.join(modeldir, "models/models_info.json")
    ...
    for k in range(1, img_num+1):
        for m in range(10):
            print(cls_name, str(k), str(m))
            # generate mask from depth
            depth_path = os.path.join(img_dir, "{:06d}_{}-depth.png".format(k, m))
            mask = depth2mask(depth_path)

            pose_path = os.path.join(img_dir, "{:06d}_{}-pose.txt".format(k, m))
            pose_info = np.loadtxt(pose_path, skiprows=1)
            R, t = pose_info[:, :3], pose_info[:, 3]
            R = np.array(R)
            R = np.reshape(R, (3, 3))  # 重整一下形状
            t = np.reshape(np.array(t), (3, 1))
            # 需要把物体的4个坐标截取下来
            xyz_info = os.path.join(rootdir, "xyz_crop_imgn", cls_name, "{:06d}_{}-xyz.pkl".format(k, m))
            if not os.path.exists(xyz_info):
                continue
            xyz = mmcv.load(xyz_info)
            x1, y1, x2, y2 = xyz["xyxy"]
            camK = intrinsic_matrix['blender'].copy()
            ...
            #  存储 Q0的坐标
            outpath = os.path.join(basic_path, "{:06d}_{}-Q0.pkl".format(k, m))
            mmcv.dump(Q0, outpath)

def run_lmimgn_Q0():
    root_dir = "/data/wanggu/Storage/lm_imgn"
    model_dir = "/data/wanggu/Storage/BOP_DATASETS/lm"
    obj_name = ["ape",
                "camera",
                "can",
                "cat",
                ...
                "lamp",
                "phone"]  # 15个分别处理
    # benchviseblue needs extra process
    for cls_name in obj_name:
        estimate_occ_mask_Q0(root_dir, model_dir, cls_name)

if __name__ == "__main__":
    root_dir = "/data/wanggu/Storage/lm_imgn"
    model_dir = "/data/wanggu/Storage/BOP_DATASETS/lm"
    obj_name = ["benchviseblue"]  # 15个分别处理
    # benchviseblue needs extra process
    for cls_name in obj_name:
        estimate_occ_mask_Q0(root_dir, model_dir, cls_name)

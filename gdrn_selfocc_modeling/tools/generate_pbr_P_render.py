...
intrinsic_matrix = {
    'linemod': np.array([[572.4114, 0., 325.26111],
                         [0., 573.57043, 242.048996],
                         [0., 0., 1.]]),
}

# 572.4114, 0.0, 325.2611082792282, 0.0, 573.57043, 242.04899594187737, 0.0, 0.0, 1.0
...
class estimate_coor_P0():
    def __init__(self, rootdir, modeldir, start_id, end_id):  # /data/wanggu//Storage/BOP_DATASETS/lm/train_pbr
        ...
        # init render
        self.ref_key = "lm_full"
        self.obj_ids = None
        self.ren = self.initRender()
    def initRender(self):
        data_ref = ref.__dict__[self.ref_key]

        classes = data_ref.objects
        model_paths = data_ref.model_paths
        # models = load_models()
        cam = data_ref.camera_matrix.astype(np.float64)
        width = data_ref.width
        height = data_ref.height
        self.obj_ids = [i for i in range(len(model_paths))]
        ren = Renderer(size=(width, height), cam=cam, model_paths=model_paths, scale_to_meter=data_ref.vertex_scale)
        return ren
    def run(self, scale=1000):
        camK = intrinsic_matrix["linemod"].astype(np.float64)
            ...
            for str_im_id in gt_dict.keys():
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    if obj_id in self.cat_ids:
                        R = np.array(anno["cam_R_m2c"], dtype="float64").reshape(3, 3)
                        t = (np.array(anno["cam_t_m2c"], dtype="float64") / 1000.0).reshape(3, 1)
                        ...
                        if mask.sum() == 0:
                            ...
                            cur_model = self.ren.models[obj_id-1]
                            self.ren.draw_model(obj_id-1, pose)
                            im, pixellist = self.ren.finish()
                            plt.imshow(pixellist)
                            plt.show()
                            pixellist = pixellist.astype(np.float32)
                            # pixellist is the result
                            P0_output = np.zeros([height, width, 3], dtype=np.float32)
                            x1, y1, x2, y2 = xyz["xyxy"]
                            for i in range(y1, y2+1):
                                for j in range(x1, x2+1):
                                    if mask[i][j] < 1 or pixellist[i, j] < 1e-5:
                                        continue
                                    else:
...
if __name__ == "__main__":
    ...
    G_P = estimate_coor_P0(root_dir, model_dir, 8, 50)
    G_P.run()

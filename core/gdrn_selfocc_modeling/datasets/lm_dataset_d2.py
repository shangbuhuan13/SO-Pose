...
class LM_Dataset(object):
    ...
    def __init__(self, data_cfg):
        ...
        self.use_cache = cfg_flag  # data_cfg.get("use_cache", True)
    ...
    def __call__(self):  # LM_Dataset
        ...
        unique_im_id = 0
        for ann_file, scene_root, xyz_root, occ_root in zip(tqdm(self.ann_files), self.image_prefixes,
                                                            self.xyz_prefixes, self.Q0_prefixes):
            ...
            for im_id in tqdm(indices):
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    if "test" not in self.name:
                        ...
                        occ_path = osp.join(occ_root, f"{int_im_id:06d}_{anno_i:06d}.pkl")  # 6个channel
                        assert osp.exists(occ_path), occ_path
                        inst["occ_path"] = occ_path
...
SPLITS_LM = dict(
    lm_13_train=dict(
        ...
        xyxzyz_prefixes=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/lm/test/Q0/{:06d}".format(ref.lm_full.obj2id[_obj]))
            for _obj in LM_13_OBJECTS
        ],
    ...
    lmo_train=dict(
        ...
        xyxzyz_prefixes=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/lm/test/Q0/{:06d}".format(ref.lmo_full.obj2id[_obj]))
            for _obj in LM_OCC_OBJECTS
        ],
        scale_to_meter=0.001,
        with_masks=True,  # (load masks but may not use it)

...
from core.gdrn_selfocc_modeling.tools.layers.layer_utils import soft_argmax
...
class GDRN_CLS2REG(nn.Module):
    def __init__(self, cfg, backbone, rot_head_net, trans_head_net=None, pnp_net=None):
        ...
        self.cfg = cfg
        self.concat = cfg.MODEL.POSE_NET.GEO_HEAD.ROT_CONCAT
    ...
    def forward(
        ...
        cfg = self.cfg
        r_head_cfg = cfg.MODEL.POSE_NET.GEO_HEAD
        ...
        if pnp_net_cfg.MASK_ATTENTION != "none":
            mask_atten = get_mask_prob(mask, mask_loss_type=cfg.MODEL.POSE_NET.LOSS_CFG.MASK_LOSS_TYPE)
        ...
        pred_rot_m = get_rot_mat(pred_rot_, pnp_net_cfg.ROT_TYPE)
    ...
    def gdrn_loss(
        ...
        r_head_cfg = cfg.MODEL.POSE_NET.GEO_HEAD
        t_head_cfg = cfg.MODEL.POSE_NET.TRANS_HEAD
        pnp_net_cfg = cfg.MODEL.POSE_NET.PNP_NET
        ...
        if not r_head_cfg.FREEZE:
            ...
            if xyz_loss_type in ["CE_coor", "CE_coor/L1"]:
                ...
                loss_dict["loss_coor_x_bin"] *= r_head_cfg.XYZ_LW
                loss_dict["loss_coor_y_bin"] *= r_head_cfg.XYZ_LW
                loss_dict["loss_coor_z_bin"] *= r_head_cfg.XYZ_LW
...
def get_xyz_mask_region_out_dim(cfg):
    ...
    xyz_loss_type = r_head_cfg.XYZ_LOSS_TYPE
    mask_loss_type = r_head_cfg.MASK_LOSS_TYPE
    if ("CE" in xyz_loss_type) or ("cls" in cfg.MODEL.POSE_NET.NAME):
...
def build_model_optimizer(cfg):
    ...
    if "resnet" in backbone_cfg.ARCH:
        ...
        rot_head_net = RotWithRegionHead(
            ...
            num_classes=r_head_cfg.NUM_CLASSES,
            rot_class_aware=r_head_cfg.XYZ_CLASS_AWARE,
        ...
        # -----------------------------------------------
        pnp_net_in_channel = 3  # NOTE: input regression results
        if pnp_net_cfg.WITH_2D_COORD:
        ...
        # build model
        model = GDRN_CLS2REG(cfg, backbone_net, rot_head_net, trans_head_net=trans_head_net, pnp_net=pnp_net)
        if cfg.MODEL.POSE_NET.USE_MTL:
            params_lr_list.append(
                {
                    "params": filter(
    ...
    if cfg.MODEL.WEIGHTS == "":
        ...
        backbone_pretrained = cfg.MODEL.POSE_NET.BACKBONE.get("PRETRAINED", "")

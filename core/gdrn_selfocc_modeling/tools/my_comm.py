...
try:
    ...
    print("You requested to import byteps which is missing or not supported for your OS.")
    BPS_AVAILABLE = False
else:
    BPS_AVAILABLE = True
...
def reduce_dict(input_dict, average=True):
    ...
    if _USE_HVD:
        ...
        return reduce_dict_bps(input_dict, average=average)
    else:
...
def reduce_dict_bps(input_dict, average=True):
    """
    Args:
        input_dict (dict): all the values will be reduced
        average (bool): whether to do average or sum
    ...
    with torch.no_grad():
        ...
        for k in sorted(input_dict.keys()):
            ...
            values.append(input_dict[k])
        values = torch.stack(values, dim=0)
        if _USE_BPS:  # NOTE: byteps
            values = bps.push_pull(values, average=average, name="reduce_dict")
        else:
            dist.all_reduce(values)
            if average:
                values /= world_size
...
def all_gather(data, group=None):
    ...
    if _USE_HVD:
        ...
        return all_gather_bps(data, group=group)
    else:
...
def all_gather_bps(data, group=None):
    global _USE_BPS
    assert _USE_BPS, f"_USE_BPS: {_USE_BPS}"
    world_size = get_world_size()
    if world_size == 1:
        return [data]
    ...
    tensor_list = [torch.empty((max_size,), dtype=torch.uint8, device=tensor.device) for _ in size_list]
    if _USE_BPS:
        # TODO: what is allgather in bps
        raise NotImplementedError()
        # NOTE: concatenated on the first dimension
        # tensor_list = bps.allgather(
        #     tensor[
        #         None,
        #     ]
        # )
...
def init_bps():
    ...
    if not BPS_AVAILABLE:
        raise RuntimeError("byteps is not available")
    else:
        bps.init()
        _USE_BPS = True
        assert torch.cuda.is_available(), "cuda is not available. Please check your installation."
